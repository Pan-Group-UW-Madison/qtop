...
function [x, objFunc, exitFlag] = gbdMasterCut(y, obj, weight, yFeasible, objFeasible, weightFeasible, vol)
    ...
    if n > 1
        stackedResult = zeros(n, size(y, 1));
        ...
        for i = 1:n
            ...
            stackedResult(i, :) = result.x;
        end

        setDifference = zeros(size(y, 1), 1);

        for i = 1:size(y, 1)

            for j = 2:n

                if (stackedResult(1, i) ~= stackedResult(j, i))
                    setDifference(i) = 1;
                    break;
                end
        ...
        differenceIdx = find(setDifference == 1);
        fixedIdx = find(setDifference == 0);

        disp(length(differenceIdx));

        volumeFixed = sum(stackedResult(1, setDifference == 0));

        l = length(differenceIdx);
        A = zeros(n + 1, l + 1);
        b = zeros(n + 1, 1);

        for i = 1:n
            A(i, 1) = -1;
            A(i, 2:end) = -weight(i, differenceIdx);
            b(i) = -obj(i) -weight(i, differenceIdx) * y(differenceIdx, i) + weight(i, fixedIdx) * (stackedResult(i, fixedIdx)' - y(fixedIdx, i));
        end

        A(n + 1, :) = ones(1, l + 1);
        A(n + 1, 1) = 0;
        b(n + 1) = vol - volumeFixed;

        targetWeight = weight(:, differenceIdx);
        targetB = b(1:n, 1);
        targetVol = vol - volumeFixed;
        disp([n, length(differenceIdx)]);
        %
        save('cons_mip.mat', 'targetWeight', 'targetB', 'targetVol');
        system('python leap_cqm_mip.py')
        load('cons_mip_result.mat');

        x = zeros(size(y, 1), 1);
        x(fixedIdx) = stackedResult(1, fixedIdx);
        x(differenceIdx) = qaResult;

        objFunc = 1e9;

        for i = 1:n
            tmpObjFunc = obj(1) - weight(1, :) * (x - y(:, 1));

            if tmpObjFunc < objFunc
                objFunc = tmpObjFunc;
            end
