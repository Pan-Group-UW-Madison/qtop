...
function [c, ce] = femAnalysis(nelx, nely, x)
    ...
    jK = reshape(kron(edofMat, ones(1, 4))', 16 * nelx * nely, 1);
    F = sparse(1:(nelx + 1) * (nely + 1), 1, 0.01, (nelx + 1) * (nely + 1), 1);
    U_base = zeros((nely + 1) * (nelx + 1), 1);
    fixeddofs = nely / 2 + 1 - floor(nely / 20):nely / 2 + 1 + floor(nely / 20);
    alldofs = 1:(nely + 1) * (nelx + 1);
    freedofs = setdiff(alldofs, fixeddofs);
    sK = reshape(KE(:) * (kmin + x(:)' * (k0 - kmin)), 16 * nelx * nely, 1);
    K = sparse(iK, jK, sK); K = (K + K') / 2;
    U_base(freedofs) = K(freedofs, freedofs) \ F(freedofs);
    ce_base = reshape(sum((U_base(edofMat) * KE) .* U_base(edofMat), 2), nely, nelx);
    c_base = sum(sum((kmin + x * (k0 - kmin)) .* ce_base));

    counter = 1;

    while error > 1e-3 && counter <= 3
        h = 2^(-counter);
        ...
        iK = reshape(kron(edofMat, ones(4, 1))', 16 * nelx * nely, 1);
        jK = reshape(kron(edofMat, ones(1, 4))', 16 * nelx * nely, 1);
        F = sparse(1:(nelx + 1) * (nely + 1), 1, 0.01, (nelx + 1) * (nely + 1), 1) * h;
        U = zeros((nely + 1) * (nelx + 1), 1);
        fixeddofs = nely / 2 + 1 - floor(nely / 20):nely / 2 + 1 + floor(nely / 20);
        alldofs = 1:(nely + 1) * (nelx + 1);
        freedofs = setdiff(alldofs, fixeddofs);
        sK = reshape(KE(:) * (kmin + xExtented(:)' * (k0 - kmin)), 16 * nelx * nely, 1);
        K = sparse(iK, jK, sK); K = (K + K') / 2;
        U(freedofs) = K(freedofs, freedofs) \ F(freedofs);
        ce = reshape(sum((U(edofMat) * KE) .* U(edofMat), 2), nely, nelx);
        ce_recast = shrinkMat(ce, 2^counter, 2^counter);
        c = sum(sum((kmin + xExtented * (k0 - kmin)) .* ce));
        c_recast = sum(sum((kmin + x * (k0 - kmin)) .* ce_recast));
